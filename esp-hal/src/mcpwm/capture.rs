...
pub use crate::pac::mcpwm0::{
    cap_ch_cfg::MODE as CaptureMode, cap_status::CAP0_EDGE as CaptureEdge,
};
use crate::{
...
impl<'d> CaptureTimer<'d> {
    ...
    pub fn start(&mut self) {
        self.cfg().modify(|_r, w| w.timer_en().bit(true));
    }

    /// Pauses the capture timer
    pub fn pause(&mut self) {
        self.cfg().modify(|_r, w| w.timer_en().bit(false));
    }
    ...
    pub fn apply_config(&mut self, config: CaptureTimerConfig) {
        self.write_phase(config.sync_phase);
    }
    ...
    pub fn trigger_sync(&mut self) {
        self.cfg().modify(|_r, w| w.sync_sw().set_bit());
    }
    ...
    pub fn set_sync_in(&mut self, sync_sel: SyncKind) {
        ...
        self.cfg().modify(|_r, w| {
            w.synci_en().bit(sync_sel != SyncKind::None);
            unsafe { w.synci_sel().bits(sync_sel as u8) }
        });
    }

    fn cfg(&self) -> &'d crate::Reg<pac::mcpwm0::cap_timer_cfg::CAP_TIMER_CFG_SPEC> {
        let info = self.mcpwm_info;
        info.regs().cap_timer_cfg()
    }

    fn phase(&self) -> &'d crate::Reg<pac::mcpwm0::cap_timer_phase::CAP_TIMER_PHASE_SPEC> {
        let info = self.mcpwm_info;
        info.regs().cap_timer_phase()
    }

    fn write_phase(&self, bits: u32) {
        #[cfg(esp32)]
        self.phase()
            .write(|w| unsafe { w.timer_phase().bits(bits) });

        #[cfg(not(esp32))]
        self.phase().write(|w| unsafe { w.phase().bits(bits) });
    }
}
